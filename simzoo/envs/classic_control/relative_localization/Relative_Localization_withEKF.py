...
def EKF(uNois, zNois, relativeState, Pmatrix, ekfStride, QR, numRob):
    ...
    for i in range(1):
        for j in [jj for jj in range(numRob) if jj != i]:
            ...
            # statPred[2] = statPred[2] % (2 * np.pi)
            # if statPred[2] > np.pi:
            #     statPred[2] = statPred[2] - np.pi * 2
            ...
            # print('K @ np.array([[resErr]]): ', K @ np.array([[resErr]]))
            # print('K: ', K)
...
class Relative_Localization_withEKF(gym.Env):
    def __init__(self):
        ...
        self.maxVel = 1  # maximum velocity [m/s]
        ...
        self.Pmatrix_ekf = np.zeros((3, 3, self.numRob, self.numRob))

        self.xEsti_rl = np.zeros(3)
        ...
        high_obs = np.array([0.1, 0.1, 0.1]) * 1.
        high_act = np.array([0.1, 0.1, 0.1]) * 0.5
    ...
    def step(self, action):  # here u1,u2=measurement, which is a result of the action
        ...
        Q = np.diag([QR[2], QR[2], QR[3], QR[2], QR[2], QR[3]]) ** 2  ########## Should it be Pxy, Pxy?????
        R = np.diag([QR[4]]) ** 2  # observation covariance
        jacoF = np.array([[1, uRi * self.dt, (-np.sin(yawij) * uVjx - np.cos(yawij) * uVjy) * self.dt],
                          [-uRi * self.dt, 1, (np.cos(yawij) * uVjx - np.sin(yawij) * uVjy) * self.dt],
                          [0, 0, 1]])
        jacoB = np.array([[-1, 0, yij, np.cos(yawij), -np.sin(yawij), 0],
                          [0, -1, -xij, np.sin(yawij), np.cos(yawij), 0],
                          [0, 0, -1, 0, 0, 1]]) * self.dt
        ...
        # print('K @ np.array([[resErr]]): ', K @ np.array([[resErr]]))
        # print('K: ', K)
        xEsti_rl_ekfcorr = xEsti_rl_pred.reshape((3, 1)) + K @ np.array([[resErr]])
        self.Pmatrix[:, :, i, j] = (np.eye(len(xEsti_rl_pred)) - K @ jacoH) @ PPred
        xEsti_rl_ekfcorr = xEsti_rl_ekfcorr.reshape(3)
        # self.xEsti_rl = xEsti_rl_ekfcorr
        # print(np.trace(self.Pmatrix[:, :, i, j])) # for tuning the filter

        # 3. RL correction
        xij = xEsti_rl_ekfcorr[0]
        yij = xEsti_rl_ekfcorr[1]
        yawij = xEsti_rl_ekfcorr[2]
        zPred = np.sqrt(xij ** 2 + yij ** 2)
        resErr = zNois[i, j] - zPred
        hat_res_xEsti_rl = resErr * np.array([u_11, u_21, u_31])  # state of RL

        self.xEsti_rl = xEsti_rl_ekfcorr + hat_res_xEsti_rl
        ...
        if step % self.ekfStride == 0:
            ...
            if self.choice == 'runInMain':
                self.relativeState_ekf = EKF(uNois, zNois, self.relativeState_ekf, self.Pmatrix_ekf, self.ekfStride, QR,
                                             self.numRob)
            else:
                self.relativeState_ekf = EKF(uNois, zNois, self.relativeState_ekf, self.Pmatrix_ekf, self.ekfStride, QR,
                                             self.numRob)
        ...
        resi_yawij = (self.xEsti_rl[2] - self.xTrueRL[2][1]) % (2 * np.pi)
        if resi_yawij > np.pi:
            resi_yawij = resi_yawij - np.pi * 2
        cost = (self.xEsti_rl[0] - self.xTrueRL[0][1])**2 + (self.xEsti_rl[1] - self.xTrueRL[1][1])**2 \
               + resi_yawij**2
        ...
        if self.choice == 'runInMain':
            return self.xEsti_ekf, self.xTrue, self.xTrueRL, self.Pmatrix, self.resErr, self.xEsti_rl, cost
        else:
...
if __name__ == '__main__':
    env = Relative_Localization_withEKF()
    T = 80
    ...
    if env.saveChoice(choice) == 'runInMain':
        ...
        estiPath_noUpdate.append(xEsti_noUpdate)
        truePath.append(xT)
        ...
        for i in range(int(T / env.dt)):
            if i > 6000:
                aa = 0
            xEsti_ekf, xTrue, xTrueRL, Pmatrix, resErr, xEsti_noUpdate, cost = env.step(np.array([0, 0, 0]))
            # print(xTrue)
            xE_ekf = xEsti_ekf[:, 1] + np.array([0., 0., 0.])
            xT = xTrueRL[:, 1] + np.array([0., 0., 0.])
            resi_yawij = (xE_ekf[2] - xT[2])%(2*np.pi)
            if resi_yawij>np.pi:
                resi_yawij = resi_yawij - np.pi*2

            Path_resi_yawij.append(resi_yawij)
            # print('xE_ekf-xT', (xE_ekf[2] - xT[2])%(2*np.pi))
            estiPath_ekf.append(xE_ekf)
            estiPath_noUpdate.append(xEsti_noUpdate)
            truePath.append(xT)
            # test
        ...
        ax.plot(steps, np.array(estiPath_noUpdate)[:, 2], color=color1, label='x2_nofusion', linestyle="dashed")
        ...
        fig = plt.figure(figsize=(9, 6))
        ax = fig.add_subplot(111)
        ax.plot(steps, np.array(Path_resi_yawij), label='resi_yawij', linestyle="dotted")
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, loc=2, fancybox=False, shadow=False)
        #     # xlim0 = 38.5
        #     # xlim1 = 40.1
        ...
        fig = plt.figure(figsize=(9, 6))
        ax = fig.add_subplot(111)
        ax.plot(steps, np.array(Path_cost), label='Path_cost', linestyle="dotted")
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, loc=2, fancybox=False, shadow=False)
        #     # xlim0 = 38.5
        #     # xlim1 = 40.1
