...
class CartPoleCost(gym.Env, Disturber):
    ...
    def step(self, action, impulse=0, process_noise=np.zeros([5])):
        ...
        if x < -self.x_threshold or x > self.x_threshold:
            a = 1
        r1 = ((self.x_threshold / 10 - abs(x - self.target_pos))) / (
            self.x_threshold / 10
        )  # -4-----1
        r2 = ((self.theta_threshold_radians / 4) - abs(theta)) / (
        ...
        # cost = 0.1 + 10 * max(
        #     0,
        #     (self.theta_threshold_radians - abs(theta)) / self.theta_threshold_radians,
    ...
    def render(self, mode="human"):
        ...
        if self.viewer is None:
            ...
            #     (self.cons_pos * scale + screen_width / 2.0, 0),
