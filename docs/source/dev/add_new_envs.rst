====================
Add new environments
====================

Please follow the steps provided in the :gymnasium:`gymnasium documentation <tutorials/gymnasium_basics/environment_creation>`
when creating a new environment. After you created your environment, you can use the :stable_gym:`stable-gym/stable_gym/__init__.py <blob/main/stable_gym/__init__.py>` 
file to register them to the ``stable_gym`` package.

stable_gym/stable_gym/envs/__init__.py
--------------------------------------

.. literalinclude:: ../../../stable_gym/__init__.py
   :language: python
   :linenos:
